...
- (void)updateSelectedView:(NSView *)view {
    if (self.selectedView != view) {
        ((HighlightingView*)self.selectedView).highlighted = NO;
        self.selectedView = view;
    }
    
    ((HighlightingView*)self.selectedView).highlighted = YES;
}
...
- (void)beginForTextField:(NSTextField *)parentTextField {
    ...
    self.localMouseDownEventMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskLeftMouseDown|NSEventMaskRightMouseDown|NSEventTypeOtherMouseDown handler:^(NSEvent *event) {
    ...
    self.lostFocusObserver = [[NSNotificationCenter defaultCenter] addObserverForName:NSWindowDidResignKeyNotification object:parentWindow queue:nil usingBlock:^(NSNotification *arg1) {
...
- (void)cancelSuggestions {
    ...
    if (self.lostFocusObserver) {
        [[NSNotificationCenter defaultCenter] removeObserver:self.lostFocusObserver];
        self.lostFocusObserver = nil;
    }
    
    if (self.localMouseDownEventMonitor) {
        [NSEvent removeMonitor:self.localMouseDownEventMonitor];
...
//    [self updateSelectedView:nil];
    ...
    if (self.viewControllers) {
        ...
        self.trackingAreas = [[NSMutableArray alloc] initWithCapacity:1];
    }
    ...
    for (NSDictionary *entry in self.suggestions) {
...
- (void)mouseUp:(NSEvent *)theEvent {
    [self.parentTextField validateEditing];
    [self.parentTextField abortEditing];
    [self.parentTextField sendAction:[self.parentTextField action] to:[self.parentTextField target]];
    [self cancelSuggestions];
}
...
- (void)moveDown:(id)sender {
    NSView *selectedView = self.selectedView;
    NSView *previousView = nil;
    for (NSViewController *viewController in [self.viewControllers reverseObjectEnumerator]) {
        NSView *view = viewController.view;
        if (view == selectedView) {
            break;
        }
